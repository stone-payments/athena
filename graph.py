...
#     try:
#         prox = paginationRepoNum(query, cursor,100)
...
#                     doc = LanguagesRepoCollection.createEdge({"size": round(((node['size']/find('totalSize', prox))*100),2)})
...
for repo in queryResult:
    try:
        ...
        try:
            ...
            doc["readme"] = True if find('ranges', prox)["endingLine"][-1] >= 15 else False
            print(find('endingLine', prox))
            doc.save()
        except Exception:
            pass
    except Exception as a:
